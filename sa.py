...
for file in name_files:
	...
	for section in pe.sections:
		...
		key = section_name[0]
		dict_file[key] = 1
		dict_file[key + '_virtual_address'] = section.VirtualAddress
		dict_file[key + '_virtual_size'] = section.Misc_VirtualSize
		dict_file[key + '_size_raw_data'] = section.SizeOfRawData

	for entry in pe.DIRECTORY_ENTRY_IMPORT:
	    dict_file[entry.dll] = 1
        for function in entry.imports:
            dict_file[function.name] = 1


	mlwr.append(dict_file)
	
# se crea el data frame
df = pd.DataFrame(mlwr)

# se guarda como csv el archivo para luego leerlo en jupyter
